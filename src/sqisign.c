...
void response_new(two_walk_long *sigma,uintbig *zip,  GEN coeff_ker_challenge_commitment, GEN I_even , const secret_key *sk, const proj *basis_two, const proj *E_cha, bool *test){
    ...
    *test = false;
    ...
    //t += clock();
    //printf("ideal to isogeny_two: %.3lf ms , test = %d \n\n",1000. * t / CLOCKS_PER_SEC, *test );
    //printf("\n");
...
void response_new_huang(two_walk_long *sigma,uintbig *zip,  GEN coeff_ker_challenge_commitment, GEN I_even , const secret_key *sk, const proj *basis_two, const proj *E_cha, bool *test){
    pari_sp ltop = avma;
    GEN A = global_setup.B;
    GEN order = global_setup.O0;
    ...
    *test = false;
    ...
    proj endpoint,A_fin;
    //clock_t t = -clock();
    ideal_to_isogeny_two_new_huang(sigma,sk->I_two,sk->I_large_prime,L_cut,last_I,&sk->phi_two,zip,test,&endpoint);
    //t += clock();
    //printf("ideal to isogeny_two: %.3lf ms , test = %d \n",1000. * t / CLOCKS_PER_SEC, *test );
    A_fin = sigma->phi[sigma->len-1].A;
    // deterministic_second_point(&Q,&sigma->phi[sigma->len-1].ker,&sigma->phi[sigma->len-1].A,two_tors_height);
...
void sign_new_huang(compressed_signature *comp_sigma, const secret_key *sk, const public_key *pk, const uintbig *m) {
    pari_sp ltop = avma;
    ...
    response_new_huang(&sigma, comp_sigma->zip, coeff_ker_challenge_commitment, I_even, sk, basis_two, &E_cha,&test);
...
//sigma1 is an isogeny from EA of degree 2^256, E1 is the codomain of sigma1.
// Initialization is built in
...
void set_secret_val(two_walk_long *sigma1, proj *E1, const public_key *mpk){
    ...
    inti_two_walk_long(sigma1,4);
    
    *E1 = mpk->E;

    index = 0;
    while (index < sigma1->len) {
        fp_random(&P.x.re);
        fp_random(&P.x.im);
        fp_random(&P.z.re);
        fp_random(&P.z.im);
        ...
        while(i<100) {
            ...
            if (mont_iszero(&Z)) {
                --index;
                *E1 = sigma1->phi[index].A;
                break;
            }

            xMUL(&P, E1, &P, &p_even_cofactor);

            xMUL(&Z, E1, &P, &pow_2_64);
            if (!mont_iszero(&Z)) {
                find = true;
                break;
            }
...
// sigma2 is an intermediate isogeny from EA to E_cha of degree 2^1000, E3 = E_com.
// sigma2: EA -> E2 of degree 2^256
// Initialization is built in
// need: pari_sp ltop = avma ?
void partial_key_ext(two_walk_long *sigma2, proj *E2, proj *E3, const public_key *mpk, const secret_key *msk,  const uintbig *ID, const proj *E1){
    ...
    GEN dlog;
    if (len_tail == 0 && !need_even_chall){
      challenge(E2, &phi_chall_even, &m, &comp_sigma.E_com, basis_plus, basis_minus, &dlog, NULL);
    }
    else {
      challenge(E2, &phi_chall_even ,&m, &comp_sigma.E_com, basis_plus, basis_minus, &dlog, basis_two);
    }
    ...
    response_new(&sigma, comp_sigma.zip, coeff_ker_challenge_commitment, I_even, msk, basis_two, E2, &test);

    if (test) {
      assert(simple_check_signature_new(&sigma,comp_sigma.zip, mpk, E2));
    }
    else {
      printf("computation failed due to norm equation failure \n");
    }
    ...
    *E3 = comp_sigma.E_com;
    ...
    //new sigma2 and E2
    inti_two_walk_long(sigma2,4);

    for(int i=0; i<4;++i) sigma2->phi[i] = sigma.phi[i];

    proj P;
    P = sigma.phi[3].ker;
    xMUL(&P, &sigma.phi[3].A, &P, &pow_2_4);
    sigma2->phi[3].ker = P;
    sigma2->phi[3].len = 61;
    
    *E2 = sigma.phi[3].A;
    eval_walk(&(sigma2->phi[3]), E2, &P);
...
void SIDH(proj *E_tar, two_walk_long *sigma1, odd_isogeny_long *eta1, const two_walk_long *sigma, const odd_isogeny_long *eta) {
	...
	isog_degree deg = eta->phi[0].deg_minus;
    ...
    */
    ...
    //generate the table
    for(i=0;i<row;++i) {
        N = (eta->phi)[i].kernel_minus;
        A0 = (eta->phi)[i].A;
        for(j=0;j<col;++j) {
            E1 = table_2[i][j].A;
            eval_walk(&(table_2[i][j]), &A0, &N);
            table_odd[i][j].A = A0;
            table_odd[i][j].kernel_minus = N;
            table_odd[i][j].kernel_plus.x = fp2_1;
            table_odd[i][j].kernel_plus.z = fp2_0;
            table_odd[i][j].deg_minus = deg;
            table_odd[i][j].deg_plus.val = 0;
        }
    ...
    N = table_odd[row-1][col-1].kernel_minus;
...
void id_commitment(id_com *com, const user_sk *usk, const public_key *mpk){
    ...
    degree_one(&deg);
    degree_set(&deg,0,65);
    for (i=1;i<p_minus_len;++i) degree_unset(&deg,i);
    
    long commitment_length = 2;

    //compute t-th commitment.
    for(t=0; t< CLI_round ; ++t) {
        inti_odd_isogeny_long(&(com[t].eta),commitment_length);
        inti_odd_isogeny_long(&(com[t].eta1),commitment_length);
        inti_odd_isogeny_long(&(com[t].eta2),commitment_length);
        ...
        while(j < commitment_length) {
            ...
            while(i<100) {
                ...
                //[p+1]P = O ?
                Z=P;
                xMUL(&Z, &(com[t].F), &Z, &p_even_cofactor);
                xMUL(&Z, &(com[t].F), &Z, &pow_2_65);
                if (mont_iszero(&Z)) {
                    --j;
                ...
                xMUL(&P, &(com[t].F), &P, &p_three_cofactor);
                xMUL(&Z, &(com[t].F), &P, &pow_3_64);
                if (!mont_iszero(&Z)) {
                    find = true;
                    break;
                }
            ...
            if (find) {
				eta_temp.A = com[t].F;
				eta_temp.kernel_minus = P;
                eta_temp.deg_minus = deg;
				eta_temp.kernel_plus.x = fp2_1;
				eta_temp.kernel_plus.z = fp2_0;
				eta_temp.deg_plus.val = 0;
            ...
            else printf("cannot find kernel of odd isogney \n");
        }
...
bool id_verify(const id_res *res, const id_com *com, const long *cha, const user_pk *upk, const public_key *mpk) {
    ...
    for(t=0;t<CLI_round;++t) {
        if(cha[t]==0) {
            len = (res[t].res1).len;
            P = (res[t].res1.phi)[len-1].ker;
            eval_walk(&((res[t].res1.phi)[len-1]), &N, &P);
            jinv256(&j1,&N);
            jinv256(&j2,&(com[t].F1));
            if(!mont_equal(&j1, &j2)) {
            ...
            len = (res[t].res2).len;
            P = (res[t].res2.phi)[len-1].ker;
            eval_walk(&((res[t].res2.phi)[len-1]), &N, &P);
            jinv256(&j1,&N);
            jinv256(&j2,&(com[t].F2));
            if(!mont_equal(&j1, &j2)) {
            ...
            len = (res[t].res3).len;
            N = (res[t].res3.phi)[len-1].A;
            P = (res[t].res3.phi)[len-1].kernel_minus;
            eval(&N, &((res[t].res3.phi)[len-1]), &P);
            jinv256(&j1,&N);
            jinv256(&j2,&(com[t].F1));
            if(!mont_equal(&j1, &j2)) {
                printf("c = %ld, res1, ", cha[t]);
            ...
            len = (res[t].res4).len;
            N = (res[t].res4.phi)[len-1].A;
            P = (res[t].res4.phi)[len-1].kernel_minus;
            eval(&N, &((res[t].res4.phi)[len-1]), &P);
            jinv256(&j1,&N);
            jinv256(&j2,&(com[t].F2));
            if(!mont_equal(&j1, &j2)) {
                printf("c = %ld, res2, ", cha[t]);
